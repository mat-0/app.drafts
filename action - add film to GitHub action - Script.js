...
if (draft.content && draft.content.trim() !== '') {
    ...
    // === GET CREDENTIALS ===
    let credential = Credential.create('GitHub Film Tracker', 'Enter your GitHub details');
    ...
    if (title && rating) {
        ...
        if (!isNaN(ratingNum) && ratingNum >= 1 && ratingNum <= 10 && rating === ratingNum.toString()) {
            ...
            if (response.success && response.statusCode === 204) {
                app.displaySuccessMessage('Film added: ' + title + ' (' + rating + ')');
                
                // Archive the draft after successful submission
                draft.isArchived = true;
                draft.update();
            } else {
                app.displayErrorMessage('GitHub API error: ' + response.statusCode);
                console.log('Response: ' + JSON.stringify(response));
                context.fail();
            }
            
        } else {
            app.displayErrorMessage('Rating must be an integer between 1 and 10');
            context.cancel();
        }
    ...
    app.displayErrorMessage('Draft is empty. Add a film title and rating first.');
    context.cancel();
}